import { CourseTaken, RequirementWithIDSourceType } from '@/types/requirements';
...
export default function buildRequirementFulfillmentGraphFromUserData(
  ...
  const requirementGraphBuilderParameters: BuildRequirementFulfillmentGraphParameters<string, CourseTaken> = {
      ...
      coursesTaken.flatMap((course) => {
        ...
        return [
          [
            ...
            {
              optIn: Array.from(
                new Set([...Object.keys(choice.arbitraryOptIn), ...choice.acknowledgedCheckerWarningOptIn])
              ),
  ...
  const dangerousRequirementFulfillmentGraph = buildRequirementFulfillmentGraph(requirementGraphBuilderParameters);
  ...
  const { doubleCountedCourseUniqueIDSet, courseToRequirementsInConstraintViolations } =
    removeIllegalEdgesFromRequirementFulfillmentGraph(safeRequirementFulfillmentGraph, (reqA, reqB) =>
      allowCourseDoubleCountingBetweenRequirements(userRequirementsMap[reqA], userRequirementsMap[reqB])
    );
