...
import userDataToExamCourses from './examUtils';
import RequirementFulfillmentGraph from './requirementGraph';
import buildRequirementFulfillmentGraphFromUserData from './requirementGraphBuilderFromUserData';
    ...
    case 'HE':
      requirement = {
        ...
        description:
          ...
          'PE courses and courses numbered 1000-1099 do not count towards the 120 credits. ' +
          'Repeated courses may not apply to this requirement, but we do not check this.',
        source: 'http://courses.cornell.edu/content.php?catoid=41&navoid=11600#Cornell_Credit_Requirements',
      };
      break;
    case 'IL':
      requirement = {
...
function mergeRequirementFulfillmentStatisticsWithAdditionalRequirements(
  ...
  function mergeRequirementFulfillmentStatistics(
    ...
    { minCountFulfilled: safeMinCountFulfilled, courses: safeCourses }: RequirementFulfillmentStatisticsWithCourses
  ...
  return {
    ...
    additionalRequirements: Object.fromEntries(
      Object.entries(dangerous.additionalRequirements).map(([key, dangerousAdditionalRequirement]) => [
        key,
        mergeRequirementFulfillmentStatistics(dangerousAdditionalRequirement, safeAdditionalRequirements[key]),
      ])
    ),
  };
}
...
export default function computeGroupedRequirementFulfillmentReports(
  ...
  const preProgramFulfillmentStatisticsMap = new Map<string, RequirementFulfillment[]>();
  userRequirements.forEach((requirement) => {
    ...
    switch (requirement.sourceType) {
      ...
      case 'preProgram': {
        const existingArray = preProgramFulfillmentStatisticsMap.get(requirement.sourceSpecificName);
        if (existingArray != null) {
          existingArray.push(fulfillmentStatistics);
        } else {
          preProgramFulfillmentStatisticsMap.set(requirement.sourceSpecificName, [fulfillmentStatistics]);
        }
        break;
      }
      default:
