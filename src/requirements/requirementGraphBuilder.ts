import { getConstraintViolations } from './utils';
import RequirementFulfillmentGraph, {
  CourseWithUniqueId,
} from './requirementGraph';
...
> = {
   ...
   */
  readonly userChoiceOnFulfillmentStrategy: Readonly<
    Record<Requirement, readonly number[]>
  >;
  /**
...
>): RequirementFulfillmentGraph<Requirement, Course> => {
  ...
  requirements.forEach((requirement) => {
    ...
    getAllCoursesThatCanPotentiallySatisfyRequirement(requirement).forEach(
      (courseId) => {
        (userCourseCourseIDToCourseMap.get(courseId) || []).forEach(
          (userCourse) => graph.addEdge(requirement, userCourse)
        );
      }
    );
  });
  ...
  userCourses.forEach((course) => {
    const userChoiceOnOptInOptOutCourse =
      userChoiceOnRequirementOverrides[course.uniqueId];
...
>(
  ...
  courseToRequirementsInConstraintViolations: Map<
    string | number,
    Set<Requirement[]>
  >;
