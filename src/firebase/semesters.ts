...
export const editSemester = (
  ...
  editSemesters((oldSemesters) => oldSemesters.map((sem) => (semesterEquals(sem, year, season) ? updater(sem) : sem)));
};
...
// exposed for testing
export const semesterEquals = (semester: FirestoreSemester, year: number, season: FirestoreSemesterSeason): boolean =>
...
export const deleteSemester = (year: number, season: FirestoreSemesterSeason, gtag?: GTag): void => {
...
export const deleteAllCoursesFromSemester = (year: number, season: FirestoreSemesterSeason, gtag?: GTag): void => {
...
export const deleteCourseFromSemesters = (courseUniqueID: number, gtag?: GTag): void => {
  ...
  editSemesters((oldSemesters) =>
    oldSemesters.map((semester) => {
      const coursesWithoutDeleted = semester.courses.filter((course) => course.uniqueID !== courseUniqueID);
...
export const getActiveSemesters = (
  ...
  if (entranceYear !== gradYear && entranceSem === 'Spring') sems.push(createSemester(entranceYear, 'Spring', []));
  if (entranceYear !== gradYear && gradSem === 'Fall') sems.push(createSemester(gradYear, 'Fall', []));
...
export const populateSemesters = (onboarding: AppOnboardingData): void => {
  ...
  const entranceSem: FirestoreSemesterSeason = onboarding.entranceSem ? onboarding.entranceSem : 'Fall';
