<template>
  <div class="semester" :class="{ 'semester--compact': compact }" data-intro-group="pageTour" data-step="3"
    :data-intro="walkthroughText()" data-disable-interaction="1" data-tooltipClass="tooltipCenter tourStep3">
    <new-course-modal @close-course-modal="closeCourseModal" @select-course="selectCourse" v-if="isCourseModalOpen"
      @add-course="addCourse" />
    <course-conflict-modal @close-course-modal="closeConflictModal" v-if="isConflictModalOpen"
      :selectedCourse="conflictCourse" :courseConflicts="courseConflicts" :selfCheckRequirements="selfCheckRequirements"
      @resolve-conflicts="handleConflictsResolved" @remove-course="deleteCourseWithoutModal" />
    <confirmation :text="confirmationText" v-if="isConfirmationOpen" />
    <delete-semester @delete-semester="deleteSemester" @close-delete-sem="closeDeleteSemesterModal"
      :deleteSemSeason="season" :deleteSemYear="year" v-if="isDeleteSemesterOpen" />
    <edit-semester @edit-semester="editSemester" @close-edit-sem="closeEditSemesterModal" :deleteSemSeason="season"
      :deleteSemYear="year" v-if="isEditSemesterOpen" />
    <clear-semester @clear-semester="clearSemester" @close-clear-sem="closeClearSemesterModal"
    ...
    <div class="semester-content">
      <div class="semester-top" :class="{ 'semester-top--compact': compact }">
        <div class="semester-left" :class="{ 'semester-left--compact': compact }">
          <span class="semester-name" data-cyId="semesterName"><img class="season-emoji" :src="seasonImg[season]"
              alt="" /> {{ season }}
            {{ year }}</span>
      ...
      <div class="semester-courses" :class="{ 'semester-hidden': isSemesterMinimized }" data-cyId="semester-courses">
        <draggable ref="droppable" class="draggable-semester-courses" group="draggable-semester-courses"
          v-model="coursesForDraggable" item-key="uniqueID"
          :componentData="{ style: { height: courseContainerHeight + 'rem' } }" @start="onDragStart" @sort="onDropped"
          @end="onDragEnd">
          <template #item="{ element }">
            <div class="semester-courseWrapper">
              <course v-if="!isPlaceholderCourse(element)" :courseObj="element" :isReqCourse="false" :compact="compact"
                :active="activatedCourse.uniqueID === element.uniqueID" class="semester-course"
                data-cyId="semester-course" :semesterIndex="semesterIndex + 1" @delete-course="deleteCourse"
                @color-course="colorCourse" @color-subject="colorSubject" @course-on-click="courseOnClick"
                @edit-course-credit="editCourseCredit" />
              <placeholder v-else :compact="compact" :semesterIndex="semesterIndex + 1" :placeholderObj="element" />
            </div>
          </template>
        </draggable>
        <add-course-button :compact="compact" @click="openCourseModal" data-cyId="semester-addCourse" />
      </div>
    </div>
    <semester-menu v-if="semesterMenuOpen" class="semester-menu"
      :isOpenModal="isDeleteSemesterOpen || isEditSemesterOpen || isClearSemesterOpen"
      @open-delete-semester-modal="openDeleteSemesterModal" @open-edit-semester-modal="openEditSemesterModal"
      @open-clear-semester-modal="openClearSemesterModal" v-click-outside="closeSemesterMenuIfOpen" />
  </div>
</template>
...
import {
  osuCourseToFirebaseSemesterCourseWithGlobalData,
  editSemester,
  editSemesters,
  addCourseToSemester,
...
export default defineComponent({
  ...
  methods: {
    ...
    addCourse(data: OSUCourse, choice: FirestoreCourseOptInOptOutChoices) {
      const newCourse = osuCourseToFirebaseSemesterCourseWithGlobalData(data);
