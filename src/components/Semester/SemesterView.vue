<template>
  <main class="semesterView" data-cyId="semesterView" :class="{
  ...
  }" @click="closeBar" :key="key">
    <div class="semesterView-top">
      <new-semester-modal @add-semester="addSemester" @close-sem-modal="closeSemesterModal"
      ...
      <div class="semesterView-settings" :class="{ 'semesterView-settings--two': noSemesters }">
        ...
        <view-dropdown data-intro-group="req-tooltip" :data-intro="getToggleTooltipText()" data-disable-interaction="1"
          data-step="4" data-tooltipClass="tooltipCenter tourStep4" :compact="compact" @click-compact="toggleCompact" />
      </div>
      ...
      <div class="semesterView-content" :class="{ 'semesterView-content--compact': compact }">
        <div v-for="(sem, semesterIndex) in semesters" :key="`${sem.year}-${sem.season}`" class="semesterView-wrapper">
          <semester ref="semester" :season="sem.season" :year="sem.year" :courses="sem.courses"
            :semesterIndex="semesterIndex" :compact="compact" :activatedCourse="activatedCourse"
            :isFirstSem="checkIfFirstSem(sem)" @course-onclick="courseOnClick" @new-semester="openSemesterModal"
...
import { addSemester, deleteSemester } from '@/firebase';
