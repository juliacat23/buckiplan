  ...
  <div class="requirementview">
    ...
    <div v-if="showMajorOrMinorRequirements">
      ...
      <div v-if="displayDetails || tourStep === 1">
        ...
        <div v-for="(requirementFulfillment, id) in partitionedRequirementsProgress.ongoing" :key="id">
          <requirement-fulfillment :requirementFulfillment="requirementFulfillment" :toggleableRequirementChoice="
            toggleableRequirementChoices[requirementFulfillment.requirement.id]
          " :color="getReqColor(req.groupName, onboardingData)" :isCompleted="false" :tourStep="tourStep"
        ...
        <div v-if="partitionedRequirementsProgress.completed.length > 0" class="row completed">
          ...
          <div class="col-1 text-right">
            <button class="btn float-right" :style="{ color: `#${getReqColor(req.groupName, onboardingData)}` }">
              <!-- Toggle to display completed reqs -->
              <p class="toggle" v-if="displayCompleted" @click="turnCompleted(false)"
                data-cyId="requirements-showCompleted">
                HIDE
              </p>
              <p class="toggle" v-else @click="turnCompleted(true)" data-cyId="requirements-showCompleted">
                SHOW
              </p>
            </button>
          </div>
        ...
        <div v-if="displayCompleted">
          <div v-for="(requirementFulfillment, id) in partitionedRequirementsProgress.completed" :key="id">
            <div class="separator" v-if="reqIndex < reqs.length - 1 || displayDetails"></div>
            <requirement-fulfillment :requirementFulfillment="requirementFulfillment" :toggleableRequirementChoice="
              toggleableRequirementChoices[requirementFulfillment.requirement.id]
            " :color="getReqColor(req.groupName, onboardingData)" :isCompleted="true" :tourStep="tourStep"
              @changeToggleableRequirementChoice="changeToggleableRequirementChoice"
              @onShowAllCourses="onShowAllCourses" />
          </div>
        </div>
