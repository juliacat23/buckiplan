  ...
  <div class="incompletesubreqcourse" v-if="courses.length > 0">
    <div class="draggable-requirements-wrapper" v-if="displayDescription">
      ...
      <div class="draggable-requirements-heading">
        ...
        <button v-if="showSeeAllLabel" class="draggable-requirements-heading-seeAll" @click="onShowAllCourses">
          {{ seeAll }}
        </button>
      </div>
      <draggable class="draggable-requirements-courses" :group="{ name: 'draggable-semester-courses', put: false }"
        :modelValue="courses" :clone="cloneCourse" item-key="code" @start="onDrag" @end="onDrop">
        <template #item="{ element }">
          <div class="requirements-courseWrapper">
            <course :courseObj="element" :isReqCourse="true" :compact="true" :active="false"
...
import { incrementUniqueID } from '@/firebase';
