<template v-if="semesters">
  <div>
    <button v-if="isMinimized && showToggleRequirementsBtn" class="requirement-sidebar-btn-open"
      @click="toggleMinimized">
      →
    </button>
    <aside v-if="isMobile ? isDisplayingMobile : !isMinimized" class="requirements">
      <div class="requirements-wrapper" data-intro-group="req-tooltip" :data-intro="getRequirementsTooltipText()"
        data-disable-interaction="1" data-step="1" data-tooltipClass="tooltipCenter tourStep1">
        <!-- loop through reqs array of req objects -->
        <div class="fixed" :class="{
          'd-none': shouldShowAllCourses,
        }" data-intro-group="req-tooltip" :data-intro="getCoursesTooltipText()" data-disable-interaction="1"
          data-step="2" data-tooltipClass="tooltipCenter tourStep2">
          <button class="requirement-debugger-toggler" v-if="debuggerAllowed" @click="toggleDebugger()">
            Open Requirement Debugger
          </button>
          <teleport-modal content-class="requirement-debugger-modal-content" :isSimpleModal="true"
          ...
          <div class="see-all-pages" v-if="numPages > 1">
            ...
            <div class="see-all-buttonWrapper">
              <button class="see-all-button" :class="{ 'see-all-button--disabled': !hasPrevPage }"
                :disabled="!hasPrevPage" @click="prevPage()">
                <span class="see-all-button-text">Prev</span>
              </button>
              <button class="see-all-button" :class="{ 'see-all-button--disabled': !hasNextPage }"
                :disabled="!hasNextPage" @click="nextPage()">
                <span class="see-all-button-text">Next</span>
              </button>
            </div>
          </div>
          <draggable :modelValue="showAllCourses.shownCourses" :clone="cloneCourse" item-key="code"
            :group="{ name: 'draggable-semester-courses', put: false }">
            <template #item="{ element }">
              <div>
                <div class="mt-3">
                  <course :courseObj="element" :compact="false" :active="false" :isReqCourse="true"
