<template>
  <div>
    <requirement-display-toggle :requirementFulfillment="requirementFulfillment" :isCompleted="isCompleted"
      :displayDescription="displayDescription" @on-toggle="toggleDescription()" />
    <div v-if="displayDescription" class="description">
      <requirement-information :requirementFulfillment="requirementFulfillment" v-model="compoundRequirementChoice"
        :color="color" />
      <div v-if="requirementFulfillment.requirement.fulfilledBy === 'toggleable'">
        <toggleable-requirement-choice-dropdown :toggleableRequirement="requirementFulfillment.requirement"
      ...
      <requirement-fulfillment-slots v-if="
        requirementFulfillment.requirement.fulfilledBy !== 'self-check' &&
        requirementFulfillment.requirement.checkerWarning == null
      " :requirementFulfillment="requirementFulfillment" :compoundRequirementChoice="compoundRequirementChoice"
        :isCompleted="isCompleted" :displayDescription="displayDescription"
        :toggleableRequirementChoice="toggleableRequirementChoice" @onShowAllCourses="onShowAllCourses" />
      <requirement-self-check-slots v-if="
        requirementFulfillment.requirement.fulfilledBy === 'self-check' ||
        requirementFulfillment.requirement.checkerWarning != null
      " :requirementFulfillment="requirementFulfillment" :isCompleted="isCompleted"
        :toggleableRequirementChoice="toggleableRequirementChoice" />
    </div>
  </div>
</template>
...
import { convertFirestoreSemesterCourseToCourseTaken } from '@/requirements/requirementUtils';
