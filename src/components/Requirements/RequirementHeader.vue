<template>
  <div class="requirementheader">
    ...
    <div v-if="req.groupName === 'College'" class="college">
      ...
      }" class="college-title-button college-title full-opacity-on-hover" :disabled="true">
        <p :style="{
          'font-weight': 500,
  ...
  'major-title-button major-title',
        ...
        }" class="major-title-top" data-cyId="majorTitle">
          {{ getMajorFullName(major) }}
        </p>
        <p :style="{
          color:
            id === displayedMajorIndex ? `#${getReqColor(req.groupName, onboardingData)}` : '',
        }" class="major-title-bottom" data-cyId="collegeTitle">
          ({{ getCollegeFullName(onboardingData.college) }})
        </p>
      </button>
    </div>
    <div v-if="reqIndex == numOfColleges + onboardingData.major.length && req.groupName === 'Minor'" class="minor">
      <button :style="{
        'border-bottom':
  ...
  'major-title major-title-button',
    ...
    <div v-if="showMajorOrMinorRequirements">
      <div class="progress">
        ...
        }" role="progressbar" aria-label="Requirements Safe Progress" :aria-valuenow="safeProgressWidthValue"
          aria-valuemin="0" aria-valuemax="100"></div>
        <div v-if="conflictsEnabled" class="progress-bar progress-bar--warning"
          :style="{ width: dangerousProgressWidth }" role="progressbar" aria-label="Requirements Conflict Progress"
          :aria-valuenow="dangerousProgressWidthValue" aria-valuemin="0" aria-valuemax="100"></div>
      </div>

      <div class="progress-text">
        <progress-bar-caution class="caution" :numConflicts="numberConflictsRounded"></progress-bar-caution>
        <span class="progress-text-credits">{{ requirementDangerouslyFulfilled }}/{{ requirementTotalRequired }}</span>
        <span class="progress-text-text"> Total Requirements Inputted on Schedule</span>
      </div>

      <!--View more college requirements -->
      <button class="btn row top view-more-dropdown" @click="toggleDetails()" aria-haspopup="true"
        data-toggle="dropdown" data-cyId="requirements-viewMore">
        <div class="col-1 p-0 btn" :style="{ color: `#${getReqColor(req.groupName, onboardingData)}` }">
          <drop-down-arrow :isFlipped="displayDetails" :fillColor="`#${getReqColor(req.groupName, onboardingData)}`"
            :isHeader="true" />
        </div>
        <div class="req-name col p-0" :style="{ color: `#${getReqColor(req.groupName, onboardingData)}` }">
